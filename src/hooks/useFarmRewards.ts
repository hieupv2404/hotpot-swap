...
export default function useFarmRewards() {
  ...
  const swapPairs = useHotpotPairs({
    chainId,
    variables: {
      where: {
  ...
  const swapPairs1d = useHotpotPairs({
    chainId,
    variables: {
      block: block1d,
      where: {
  ...
  const masterChefV1HotpotPerBlock = useMasterChefV1HotpotPerBlock()
