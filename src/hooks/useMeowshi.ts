...
import { useMeowshiContract, useHotpotBarContract, useHotpotContract } from './useContract'
...
const useMeowshi = (sushi: boolean) => {
  ...
  const sushiContract = useHotpotContract(true)
  const barContract = useHotpotBarContract(true)
  ...
  const meowHotpot = useCallback(
    async (amount: BalanceProps | undefined) => {
      if (amount?.value) {
        try {
          const tx = await meowshiContract?.meowHotpot(account, amount?.value)
          addTransaction(tx, { summary: 'Enter Meowshi' })
          return tx
        } catch (e) {
          return e
  ...
  const unmeowHotpot = useCallback(
    async (amount: BalanceProps | undefined) => {
      if (amount?.value) {
        try {
          const tx = await meowshiContract?.unmeowHotpot(account, amount?.value)
          addTransaction(tx, { summary: 'Leave Meowshi' })
          return tx
        } catch (e) {
          return e
