import { useBlock, useHotpotPairs } from '../../../services/graph'
...
export default function Pairs() {
  ...
  const pairs = useHotpotPairs({ chainId })
  const pairs1d = useHotpotPairs({ variables: { block: block1d }, shouldFetch: !!block1d, chainId })
  const pairs2d = useHotpotPairs({ variables: { block: block2d }, shouldFetch: !!block2d && type !== 'all', chainId }) // No need to fetch if we don't need the data
  const pairs1w = useHotpotPairs({ variables: { block: block1w }, shouldFetch: !!block1w, chainId })
  const pairs2w = useHotpotPairs({ variables: { block: block2w }, shouldFetch: !!block2w && type !== 'all', chainId })
