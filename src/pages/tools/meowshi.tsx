...
import { ChainId, Currency, Token } from '@hotpot-swap/core-sdk'
import { MEOW, HOTPOT, XHOTPOT } from '../../config/tokens'
...
import useMeowshiPerXHotpot from '../../hooks/useMeowshiPerXHotpot'
...
export default function Meowshi() {
  ...
  const sushiPerXHotpot = useHotpotPerXHotpot()
  const [meowshiPerXHotpot, xHotpotPerMeowshi] = useMeowshiPerXHotpot()
  ...
  const [currencies, setCurrencies] = useState({
    [Field.INPUT]: HOTPOT[ChainId.ETHEREUM],
    [Field.OUTPUT]: MEOW,
  })
  ...
  const handleInput = useCallback(
    async (val, field) => {
      setFields((prevState) => {
        const inputRate =
          currencies[Field.INPUT] === XHOTPOT
            ? meowshiPerXHotpot.mul(e10(5))
            : meowshiPerXHotpot.mul(e10(5)).mulDiv(e10(18), sushiPerXHotpot.toString().toBigNumber(18))
        const outputRate =
          currencies[Field.OUTPUT] === XHOTPOT
            ? xHotpotPerMeowshi.div(e10(5))
            : xHotpotPerMeowshi.mulDiv(sushiPerXHotpot.toString().toBigNumber(18), e10(18)).div(e10(5))
    ...
    [currencies, meowshiPerXHotpot, sushiPerXHotpot, xHotpotPerMeowshi]
  )
  ...
  return (
    <Container id="meowshi-page" className="py-4 md:py-8 lg:py-12" maxWidth="2xl">
      <Head>
        <title>Meowshi | Hotpot</title>
        <meta key="description" name="description" content="HotpotSwap Meowshi..." />
      </Head>
      ...
      <div className="z-0 relative mb-[-38px] md:mb-[-54px] ml-0 md:ml-4 flex justify-between gap-6 items-center">
        ...
        <div className="bg-[rgba(255,255,255,0.04)] p-4 py-2 rounded flex flex-row items-center gap-4 mb-[54px]">
          ...
          <Typography variant="xs" weight={700}>
            {i18n._(t`MEOW tokens wrap xHOTPOT into BentoBox for double yields and can be
      ...
      <div className="relative grid gap-4 p-4 border-2 rounded z-1 bg-dark-900 shadow-swap border-dark-800">
        ...
        <div className="relative mt-[-24px] mb-[-24px] ml-[28px] flex items-center">
          ...
          <Typography variant="sm" className="text-secondary ml-[26px]">
            ...
            {(currencies[Field.INPUT] === HOTPOT[ChainId.ETHEREUM] ||
              currencies[Field.OUTPUT] === HOTPOT[ChainId.ETHEREUM]) &&
              ' xHOTPOT → '}
