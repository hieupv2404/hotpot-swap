...
export function useHotpotPrice(variables = undefined, swrConfig: SWRConfiguration = undefined) {
  const { data } = useSWR(['sushiPrice', stringify(variables)], () => getHotpotPrice(variables), swrConfig)
  return data
}
...
export function useHotpotPairs({
  chainId = ChainId.ETHEREUM,
  variables,
  shouldFetch = true,
  swrConfig = undefined,
