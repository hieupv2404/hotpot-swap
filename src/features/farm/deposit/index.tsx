...
import { MASTER_CHEF, MASTER_CHEF_V2, HOTPOT_TOKEN } from '../../../constants/addresses'
...
export const hotpotardData = (data: any) => {
...
export default function Deposit({
  ...
  const [tokenHotpotPrice, setTokenHotpotPrice] = useState(0)
  ...
  useEffect(() => {
    setCountValue(null)
    const getTokenHotpotPrice = async () => {
      const [hotpotPrice, tokenBalance] = await Promise.all([0, lpTokenContract.methods.balanceOf(account).call()])
      ...
      setTokenHotpotPrice(hotpotPrice)
    }

    isOpenDeposit && getTokenHotpotPrice()
  ...
  const calculateSdcEarnedPerThousandDollars = ({ numberOfDays, farmApy, tokenPrice }) => {
    ...
    //   Calculate the starting HOTPOT balance with a dollar balance of $1000.
    ...
    // To get the HOTPOT earned, deduct the amount after compounding (finalAmount) from the starting HOTPOT balance (principal)
  ...
  const tokenHotpotEarnedPerThousand1D = calculateSdcEarnedPerThousandDollars({
    numberOfDays: 1,
    farmApy,
    tokenPrice: tokenHotpotPrice,
  })
  const tokenHotpotEarnedPerThousand30D = calculateSdcEarnedPerThousandDollars({
    numberOfDays: 30,
    farmApy,
    tokenPrice: tokenHotpotPrice,
  })
  const tokenHotpotEarnedPerThousand365D = calculateSdcEarnedPerThousandDollars({
    numberOfDays: 365,
    farmApy,
    tokenPrice: tokenHotpotPrice,
  })

  const handlePriceAmount = (tokenHotpotEarned, tokenHotpotPrice) => {
    return hotpotardData(new BigNumber(tokenHotpotEarned).times(tokenHotpotPrice).toNumber()) === 0 ||
      hotpotardData(new BigNumber(tokenHotpotEarned).times(tokenHotpotPrice).toNumber()) === '0'
      ? '0.00'
      : hotpotardData(new BigNumber(tokenHotpotEarned).times(tokenHotpotPrice).toNumber())
  }
    ...
    <Modal isOpen={isOpenDeposit} onDismiss={onDismissDeposit} maxWidth={568}>
      ...
      <ContentModal>
        <div className="head-title">
          ...
          <div className="available" onClick={() => setCountValue(hotpotardData(userLpAmount))}>
            {`${
              farmType === FarmTypeEnum.TOKEN ? 'HOTPOT' : `LP-${farm.pair.token.symbol}-${farm.pair.quoteToken.symbol}`
            } ${i18n._(t`Balance:`)} `}
            <StyledNewTooltip>
              <NewTooltip
                dataTip={hotpotardData(userLpAmount)}
                dataValue={handleAdvancedDecimal(hotpotardData(userLpAmount), 6)}
              />
            </StyledNewTooltip>
          </div>
        </div>
        <PoolToken>
          ...
          <div className="count-tk">
            <NewTooltip
              className="count-tt"
              dataTip={`~${hotpotardData(
                farm.type === FarmTypeEnum.TOKEN ? Number(countValue) * tokenPriceUSD : Number(countValue) * lpPrice
              ).toFixed(2)}`}
              dataValue={`~${hotpotardData(
                farm.type === FarmTypeEnum.TOKEN ? Number(countValue) * tokenPriceUSD : Number(countValue) * lpPrice
              ).toFixed(2)}`}
            />
          ...
          <div className="flex justify-between py-1">
            ...
            <CountStake>
              <NewTooltip
                dataTip={
                  '$' +
                  (hotpotardData(userStake) === 0 || hotpotardData(userStake) === '0'
                    ? '0.00'
                    : hotpotardData(userStake))
                }
                dataValue={
                  '$' +
                  (hotpotardData(userStake) === 0 || hotpotardData(userStake) === '0'
                    ? '0.00'
                    : handleAdvancedDecimal(hotpotardData(userStake), 6))
                }
              />
            </CountStake>
          </div>
            ...
            <tr className="daily-earning">
              <td>
                <TextEarning>
                  ...
                  <NewTooltip
                    dataTip={i18n._(t`Estimated daily earnings based on APY with 1000$ deposit`)}
              ...
              <td>
                <TextStake>
                  {isMobile ? (
                    <NewTooltip
                      dataTip={hotpotardData(new BigNumber(tokenHotpotEarnedPerThousand1D).toNumber())}
                      dataValue={
                        handleAdvancedDecimal(
                          toFixed(hotpotardData(new BigNumber(tokenHotpotEarnedPerThousand1D).toNumber())),
                          2
                        ) + ' HOTPOT'
                      }
                    />
              ...
              <td>
                <CountStake>
                  {isMobile ? (
                    <NewTooltip
                      dataTip={'$' + handlePriceAmount(tokenHotpotEarnedPerThousand1D, tokenHotpotPrice)}
                      dataValue={
                        '$' +
                        handleAdvancedDecimal(
                          toFixed(handlePriceAmount(tokenHotpotEarnedPerThousand1D, tokenHotpotPrice)),
                          2
                        )
                      }
                    />
            ...
            <tr className="monthly-earning">
              <td>
                <TextEarning>
                  ...
                  <NewTooltip
                    dataTip={i18n._(t`Estimated monthly earnings based on APY with 1000$ deposit`)}
                    dataValue={exclamationMark()}
                  />
                </TextEarning>
              </td>
              <td>
                <TextStake>
                  {isMobile ? (
                    <NewTooltip
                      dataTip={hotpotardData(new BigNumber(tokenHotpotEarnedPerThousand30D).toNumber())}
                      dataValue={
                        handleAdvancedDecimal(
                          toFixed(hotpotardData(new BigNumber(tokenHotpotEarnedPerThousand30D).toNumber())),
                          2
                        ) + ' HOTPOT'
                      }
                    />
              ...
              <td>
                <CountStake>
                  {isMobile ? (
                    <NewTooltip
                      dataTip={'$' + handlePriceAmount(tokenHotpotEarnedPerThousand30D, tokenHotpotPrice)}
                      dataValue={
                        '$' +
                        handleAdvancedDecimal(
                          toFixed(handlePriceAmount(tokenHotpotEarnedPerThousand30D, tokenHotpotPrice)),
                          2
                        )
                      }
                    />
            ...
            <tr className="yearly-earning">
              <td>
                <TextEarning>
                  ...
                  <NewTooltip
                    dataTip={i18n._(t`Estimated yearly earnings based on APY with 1000$ deposit`)}
                    dataValue={exclamationMark()}
                  />
                </TextEarning>
              </td>
              <td>
                <TextStake>
                  {isMobile ? (
                    <NewTooltip
                      dataTip={hotpotardData(new BigNumber(tokenHotpotEarnedPerThousand365D).toNumber())}
                      dataValue={
                        handleAdvancedDecimal(
                          toFixed(hotpotardData(new BigNumber(tokenHotpotEarnedPerThousand365D).toNumber())),
                          2
                        ) + ' HOTPOT'
                      }
                    />
              ...
              <td>
                <CountStake>
                  {isMobile ? (
                    <NewTooltip
                      dataTip={'$' + handlePriceAmount(tokenHotpotEarnedPerThousand365D, tokenHotpotPrice)}
                      dataValue={
                        '$' +
                        handleAdvancedDecimal(
                          toFixed(handlePriceAmount(tokenHotpotEarnedPerThousand365D, tokenHotpotPrice)),
                          2
                        )
                      }
                    />
            ...
            <div className="label">
              {Number(countValue) === 0
                ...
                ? `${
                    farmType === FarmTypeEnum.TOKEN
                      ? 'Approve HOTPOT'
                      : `${i18n._(t`Approve LP`)} ${farm.pair.token.symbol}-${farm.pair.quoteToken.symbol}`
                  }`
                : approval === ApprovalState.PENDING
                ? `${
                    farmType === FarmTypeEnum.TOKEN
                      ? 'Approving HOTPOT'
                      : `${i18n._(t`Approving LP`)} ${farm.pair.token.symbol}-${farm.pair.quoteToken.symbol}`
                  }`
                : i18n._(t`Deposit`)}
            </div>
          </button>
