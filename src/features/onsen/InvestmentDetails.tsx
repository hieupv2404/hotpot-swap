...
import { CurrencyAmount, JSBI, Token, USD, ZERO } from '@hotpot-swap/core-sdk'
import { getAddress } from '@ethersproject/address'
import { PairType } from './enum'
import { usePendingHotpot, useUserInfo } from './hooks'
...
const InvestmentDetails = ({ farm }) => {
  ...
  const pendingHotpot = usePendingHotpot(farm)
  ...
  const rewardValue =
    (farm?.rewards?.[0]?.rewardPrice ?? 0) * Number(pendingHotpot?.toExact() ?? 0) +
    ...
    <div className="flex flex-col w-full space-y-8">
      ...
      <div className="flex flex-col w-full space-y-4">
        <div className="flex items-end justify-between">
          ...
          {((pendingHotpot && pendingHotpot.greaterThan(ZERO)) || (pendingReward && Number(pendingReward) > 0)) && (
            <button
            ...
            {farm?.rewards?.map((reward, i) => (
              <div key={i} className="flex items-center space-x-2">
                <Image
                  ...
                  alt={reward.token}
                />
                {i === 0 && <Typography>{formatNumber(pendingHotpot?.toSignificant(6) ?? 0)}</Typography>}
