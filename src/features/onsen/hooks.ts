...
export function usePendingHotpot(farm) {
  ...
  const result = useSingleCallResult(args ? contract : null, 'pendingHotpot', args)?.result
...
export function useChefPositions(contract?: Contract | null, rewarder?: Contract | null, chainId = undefined) {
  ...
  const pendingHotpot = useSingleContractMultipleData(args ? contract : null, 'pendingHotpot', args)
  ...
  return useMemo(() => {
    if (!pendingHotpot || !userInfo) {
      return []
    }
    return zip(pendingHotpot, userInfo)
      .map((data, i) => ({
        id: args[i][0],
        pendingHotpot: data[0].result?.[0] || Zero,
        amount: data[1].result?.[0] || Zero,
        chef: getChef(),
        // pendingTokens: data?.[2]?.result,
      }))
      .filter(({ pendingHotpot, amount }) => {
        return (pendingHotpot && !pendingHotpot.isZero()) || (amount && !amount.isZero())
      })
  }, [args, getChef, pendingHotpot, userInfo])
}
