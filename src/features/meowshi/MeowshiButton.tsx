...
const MeowshiButton: FC<MeowshiButtonProps> = ({ meowshiState }) => {
  ...
  const sushiBalance = useTokenBalance(account, HOTPOT[ChainId.ETHEREUM])
  const xHotpotBalance = useTokenBalance(account, XHOTPOT)
  const { approvalState, approve, meow, unmeow, meowHotpot, unmeowHotpot } = useMeowshi(
  ...
  const handleSubmit = async () => {
    ...
    let tx
    if (doMeow) {
      if (currencies[Field.INPUT]?.symbol === 'HOTPOT') {
        tx = await meowHotpot({
          value: parseUnits(fields[Field.INPUT], sushiBalance.currency.decimals),
          decimals: sushiBalance.currency.decimals,
        })
      }
      if (currencies[Field.INPUT]?.symbol === 'xHOTPOT') {
        tx = await meow({
          value: parseUnits(fields[Field.INPUT], sushiBalance.currency.decimals),
          decimals: xHotpotBalance.currency.decimals,
        })
      }
    } else {
      if (currencies[Field.OUTPUT]?.symbol === 'HOTPOT') {
        tx = await unmeowHotpot({
          value: parseUnits(fields[Field.INPUT], sushiBalance.currency.decimals),
          decimals: xHotpotBalance.currency.decimals,
        })
      }
      if (currencies[Field.OUTPUT]?.symbol === 'xHOTPOT') {
        tx = await unmeow({
          value: parseUnits(fields[Field.INPUT], sushiBalance.currency.decimals),
          decimals: xHotpotBalance.currency.decimals,
        })
