import { useBlock, useDayData, useHotpotPairs } from '../../../../services/graph'
...
export default function PairChartCard(props: PairChartCardProps): JSX.Element {
  ...
  const pair = useHotpotPairs({ variables: { where: { id: props.pair } }, shouldFetch: !!props.pair, chainId })?.[0]

  const pair1d = useHotpotPairs({
    chainId,
    variables: { block: block1d, where: { id: props.pair } },
    shouldFetch: !!props.pair && !!block1d,
  })?.[0]

  const pair2d = useHotpotPairs({
