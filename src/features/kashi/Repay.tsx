...
import {
  Percent,
  HOTPOTSWAP_MULTISWAPPER_ADDRESS,
  HOTPOTSWAP_MULTI_EXACT_SWAPPER_ADDRESS,
  WNATIVE,
...
export default function Repay({ pair }: RepayProps) {
  ...
  async function onExecute(cooker: KashiCooker) {
    ...
    if (swap && trade) {
      ...
      cooker.bentoTransferCollateral(pair.userCollateralShare, HOTPOTSWAP_MULTI_EXACT_SWAPPER_ADDRESS[chainId || 1])
      ...
      cooker.action(
        HOTPOTSWAP_MULTI_EXACT_SWAPPER_ADDRESS[chainId || 1],
        ZERO,
