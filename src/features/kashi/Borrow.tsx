...
import { Percent, HOTPOTSWAP_MULTISWAPPER_ADDRESS, WNATIVE } from '@hotpot-swap/core-sdk'
  ...
  async function onExecute(cooker: KashiCooker): Promise<string> {
    ...
    if (borrowValueSet) {
      ...
      cooker.borrow(
        ...
        swap || useBentoBorrow,
        swap ? HOTPOTSWAP_MULTISWAPPER_ADDRESS[chainId || 1] : ''
      )
    }
    if (borrowValueSet && trade) {
      ...
      cooker.action(
        HOTPOTSWAP_MULTISWAPPER_ADDRESS[chainId || 1],
        ZERO,
