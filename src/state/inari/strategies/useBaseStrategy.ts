...
const useBaseStrategy = ({ id, general, tokenDefinitions }: useBaseStrategyInterface): BaseStrategyHook => {
  ...
  // Look at useStakeHotpotToCreamStrategy for an example
  ...
  // This one is converting Hotpot to xHotpot and vice-versa.
  ...
  const calculateOutputFromInput = useCallback(
    (zapIn: boolean, inputValue: string, inputToken: Token, outputToken: Token) => {
      if (!sushiPerXHotpot || !inputValue) return null

      return (
        zapIn
          ? inputValue.toBigNumber(18).mulDiv(e10(18), sushiPerXHotpot.toString().toBigNumber(18))
          : inputValue.toBigNumber(18).mulDiv(sushiPerXHotpot.toString().toBigNumber(18), e10(18))
      )?.toFixed(18)
    },
    [sushiPerXHotpot]
  )
